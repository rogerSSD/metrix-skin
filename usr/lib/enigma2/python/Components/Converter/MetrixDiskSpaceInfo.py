...
class MetrixDiskSpaceInfo(Poll, Converter):
	HDDTEMP = 0
	LOADAVG = 1
	MEMTOTAL = 2
	MEMFREE = 3
	...
	def getText(self):
		...
		if self.type == self.HDDTEMP:
			...
			if list[0] == 0:
				...
				text = "%s, in use: %s%%" % (self.getSizeStr(list[0]), list[3])
			elif self.fullFormat:
				text = "%s Free: %s Used: %s (%s%%)" % (self.getSizeStr(list[0]), self.getSizeStr(list[2]), self.getSizeStr(list[1]), list[3])
			else:
				text = "%s free of %s" % (self.getSizeStr(list[2]), self.getSizeStr(list[0]))
		return text
